...
/**
 * External function: start a new conversation and send its first message.
 *
 * @package    mod_coursemail
 * @copyright  2026 Jose Luis Simon
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class start_conversation extends external_api {
    /**
     ...
     * @param string $recipienttype Recipient targeting mode.
     * @param int[] $recipientids Ids depending on the mode.
    ...
    public static function execute(
        ...
        if (in_array($params['recipienttype'], ['staff', 'staffselected'], true)) {
            ...
            // Students write to teachers; these are staff-only conversation options.
            $params['requiresresponse'] = false;
            $params['requiresmanualcomplete'] = false;
        }

        $recipientids = recipients::resolve($cm, $context, $USER->id, $params['recipienttype'], $params['recipientids']);
        ...
        $message = $mailbox->start_conversation(
            $USER->id,
            $params['subject'],
            $params['requiresresponse'],
            $recipientids,
            $params['body'],
            $params['bodyformat'],
            $params['requiresmanualcomplete']
        );
